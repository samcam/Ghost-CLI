    ...
    async run(argv) {
        ...
        if (semver.lt(instance.cliVersion, '1.0.0')) {
            this.ui.log(
                ...
                `For instructions on how to upgrade your instance, visit ${chalk.blue.underline('https://ghost.org/faq/upgrading-from-deprecated-ghost-cli/')}.\n`,
                'yellow'
            );
        }
        ...
        const tasks = [{
            ...
            task: releaseNotes.displayReleaseNotes,
            enabled: () => process.env.GHOST_SKIP_RELEASE_NOTES !== 'true'
        }, {
            title: 'Checking release notes',
            task: releaseNotes.checkForConfirmation,
            enabled: () => context.releaseNotesContainWarning
        }, {
